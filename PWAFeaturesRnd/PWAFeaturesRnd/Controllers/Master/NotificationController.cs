...
namespace PWAFeaturesRnd.Controllers.Master
{
    ...
    public class NotificationController : BaseController
    {
        ...
        public async Task<IActionResult> GetChannelMessagesForOfflineServe(string sessionDetails)
        {
            ...
            DataTablePageRequest<string> pageRequest = new DataTablePageRequest<string>();
            pageRequest.Length = Int32.MaxValue;
            pageRequest.Start = 1;
            pageRequest.Columns = new List<Column>();
            pageRequest.Order = new List<Order>();
            pageRequest.Columns = new List<Column>();
            ...
            DataTablePageRequest<string> MessagePageRequest = new DataTablePageRequest<string>();
            MessagePageRequest.Length = Int32.MaxValue;
            MessagePageRequest.Start = 100;
            MessagePageRequest.Columns = new List<Column>();
            MessagePageRequest.Columns.Add(new Column() { Name = "ParticipantName" });

            MessagePageRequest.Order = new List<Order>();
            MessagePageRequest.Order.Add(new Order()
            {
                Column = 0,
                Dir = "asc"
            });
            List<dynamic> channelMessage = new List<dynamic>();

            Int32.TryParse(cookieValueFromReq, out timeZoneOffSet);
            int i = 1;
            foreach (var channel in response.Results)
            {
                PagedResult<PagedList<PWAFeaturesRnd.ViewModels.Notification.NotificationUserMessageViewModel>> MessageResponse = await _client.GetChannelMessages(pageRequest, channel.ChannelId.ToString(), timeZoneOffSet);
                if (MessageResponse != null && MessageResponse.Results != null && MessageResponse.Results.Any())
                {
            ...
            return new JsonResult(new { data = channelMessage });
        }
        ...
        /// <param name="sessionDetails"></param>
        /// <returns></returns>
        public async Task<JsonResult> GetChannelListForOfflineServe(ChannelRequest channelRequest, string sessionDetails)
        {
            ChannelRequest filter = CommonUtil.GetSessionStorageFilter<ChannelRequest>(_provider, sessionDetails);
            if (filter != null)
            {
